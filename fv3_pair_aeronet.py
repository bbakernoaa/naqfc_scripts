...
def  pair_point(da,df,sub_map,interp):
     dfpair=da.monet.combine_point(df,mapping_table=sub_map,method=interp)
#     print(dfpair)
     return dfpair
...
#    parser.add_argument('-e', '--enddates',    help='string input end date for pairing YYYY-MM-DD HH:MM:SS', type=str, required=True)
    ...
    parser.add_argument('-o', '--output',      help='string output path for paired dataframe, stats, plots', type=str, required=False,default='paired_output.hdf')
